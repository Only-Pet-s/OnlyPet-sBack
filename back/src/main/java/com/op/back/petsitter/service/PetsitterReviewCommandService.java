...
public class PetsitterReviewCommandService {
    ...
    public void createReview(
        ...
        if (LocalDateTime.of(date, endTime).isAfter(LocalDateTime.now())) {
            throw new ReviewException("아직 돌봄이 완료되지 않았습니다.");
        }
        ...
        try {
            firestore.runTransaction(tx -> {
                ...
                // 추후 내정보에서 작성했던 리뷰 조회를 위해 users/psReviews에 저장
    ...
    public void updateReview(
        ...
        DocumentReference psReviewRef = firestore.collection("petsitters") // 펫시터 리뷰 참조
                .document(petsitterId)
                .collection("reviews")
                .document(reviewId);
        ...
        try {
            firestore.runTransaction(tx -> {
                ...
                if (reviewCount <= 1) {
                    ...
                    double newAvgRating = ((currAvgRating * reviewCount) - rating) / (reviewCount - 1); // 평점 평균 계산
