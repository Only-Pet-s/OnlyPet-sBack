...
public class ShortsController {
        ...
        // 북마크 추가
        @PostMapping("/{shortsId}/bookmark")
        public ResponseEntity<Void> bookmark(@PathVariable String shortsId)
                        throws Exception {
                shortsService.bookmarkShorts(shortsId, currentUid());
                return ResponseEntity.ok().build();
        }

        // 북마크 취소
        @DeleteMapping("/{shortsId}/bookmark")
        public ResponseEntity<Void> unbookmark(@PathVariable String shortsId)
                        throws Exception {
                shortsService.unbookmarkShorts(shortsId, currentUid());
                return ResponseEntity.ok().build();
        }
